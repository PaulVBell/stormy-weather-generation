...
The following code chunks take a few hours to run on 4 cores as k-fold cross validation fits the same brms model k times. To avoid refitting the model load the "storm_wg_fit.RData" file which contains the fitted model.
...
The 10 folds are already included as a column in the data dataframe for reproducibility but to increase computation speed use fewer numbers of folds. Alternatively reduce the number of stations.
...
```{r occurrence_fit}
# # Uncomment to rerun fitting the occurrence model.
# 
# stations <- c("wattisham", "tiree", "plymouth")
# occ_fit_list <- list()
...
```{r look_at_list}
names(occ_fit_list$wattisham$yes)
```
...
The Amounts CRPS values are below. Note Wattisham is not inproved much at all.
...
Here we see miscalibration in the amounts model for all stations, regardless of storm types.
...
Plotting for Wattisham without storm types the draws from the PPD (yrep) versus observations (y):

```{r}
draw_df %>%
  filter(station == "wattisham") %>%
  filter(types == "no") %>%
  ggplot() +
...
Similarly for all stations with and without storm types:
...
Similarly to the PIT historgram the main miscalibration is present in the amounts model.
...
The same for amounts. Note that using N_eff here isn't needed as there are so many "missing" values, i.e. times when precipitation doesn't occur Also note the low DM score for Wattisham.
...
# Comparison between 1 mm / 6hr and 0 mm / 6hr threshold, calibration

The choice of 1mm / 6hr as the occurrence threshold was made because of calibration of the amounts model. A Gamma distribution was found to be poorly calibrated to precipitation amounts, when precipitation below 1 mm was considered.

Below show an example of this miscalibration for the Wattisham station with storm types.

```{r get_d0mm}
d0mm <- data[["wattisham"]] %>%
  mutate(occur = as.numeric(prcp_amt > 0))
```

Below fits the occurrence and amounts model for a 0mm / 6hr threshold.

As above, the models have been fitted separately and the outputs saved and these can be loaded below.

```{r load_fit_0mm}
# Load the 0mm threshold weather generator fit into the environment.
load("fits/fit_wattisham_0mm.RData")
```

```{r fit_0mm}
# # Uncomment and run to fit the occurrence and amounts models for Wattisham, with storm types
# 
# occ_fit_0mm <- occ_fit_kfold_brms(
#     data = d0mm,
#     formula = occurrence_formula_with_st,
#     priors = occurrence_priors_with_st,
#     iter = 2000,
...
#     )
# gc()
# amt_fit_0mm <- amt_fit_kfold_brms(
#     data = d0mm,
#     formula = prcp_amt ~ -1 + (sin_doy + cos_doy + w500 + q850) * storm_type - sin_doy - cos_doy - w500 - q850,
#     priors = amounts_priors_with_st,
#     iter = 2000,
#     cores = 4
#     )
...
```{r save_fit_0mm}
# # Uncomment and run to save the 0mm threshold weather generator fit as a .RData file.
# save(occ_fit_0mm, amt_fit_0mm, file = "fit_wattisham_0mm.RData")
```

The PIT histogram for Wattisham is below. The model with a threshold of 0 mm / 6hrs is clearly miscalibrated for low and high precipitation.

```{r get_PIT_0mm}
yrep <- amt_fit_0mm$yrep[,which(amt_fit_0mm$y > 0)]
y <- amt_fit_0mm$y[which(amt_fit_0mm$y > 0)]
      
amt_PIT_0mm <- apply(t(yrep) - y, MARGIN = 1, FUN = get_PIT)
```

```{r}
data.frame(PIT = amt_PIT_0mm) %>%
  ggplot() +
  geom_density(aes(x = PIT, linetype = "0 mm / 6hr"), bounds = c(0,1)) +
  geom_density(data = filter(df_amt_pit, station == "wattisham"),
               aes(x = yes, linetype = "1 mm / 6hr"), bounds = c(0,1))+
  geom_hline(aes(yintercept = 1), linetype = "dashed", color = "red") + 
  scale_linetype_manual(name = "threshold", values = c(`1 mm / 6hr` = 1, `0 mm / 6hr` = 2))
```
